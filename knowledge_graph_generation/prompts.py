...
examples = [
    {
        "text": (
            "The present invention provides an apparatus for facilitating a photovoltaic device to "
            "provide a wireless communication channel. The apparatus comprises a switch connected in "
            "parallel with the photovoltaic device and configured for driving the photovoltaic device "
            "to produce optical signals carrying sensed data to be transmitted; and a control module "
            "connected with the switch and configured for receiving electrical sensing signals and "
            "generate a control signal to control the switch."
        ),
    ...
    {
        "text": (
            "Inventor(s): Garaj; Martin (Bratislava, SK), Chung; Shu Hung Henry (Hong Kong, HK)\n"
    ...
    {
        "text": (
            "Applicant: City University of Hong Kong, Hong Kong (HK)\n"
            "US Patent No.: 11671731"
        ),
        "head": "11671731",
        "head_type": "Patent",
    ...
    {
        "text": (
            "US Patent 9413457 is cited in this invention as prior art.\n"
            "US Patent No.: 11671731"
        ),
        "head": "11671731",
        "head_type": "Patent",
...
def create_unstructured_prompt(
    ...
    base_string_parts = [
        ...
        "output in a JSON format containing a list with JSON objects. Each "
        'object should have the keys: "head", "head_type", "relation", "tail", '
        'and "tail_type". The "head" key is the head of the relation and must '
        f"be an extracted entity with one of the types from {node_labels_str}. "
        'The "head_type" key is the type of the "head" key, and must be one of '
        f"the types from {node_labels_str}."
        if node_labels
        else "",
        f'The "relation" key is the type of relation between the "head" and the '
        f'"tail", and must be one of the relations from {rel_types_str}.'
        if rel_types
        else "",
        'The "tail" key is the tail of the relation and must be an extracted '
        f'entity with one of the types from {node_labels_str}. The "tail_type" '
        'key is the type of the "tail" key, and must be one of the types from '
        f"{node_labels_str}."
        if node_labels
        else "",
    ...
    human_string_parts = [
        "Your task is to extract entities and relations from the provided text.\n\n"
        ...
        'The relations defined in "RELATION TYPES" can only appear '
        f"between specific node types, which are all listed in {rel_types}.\n"
        f"DO NOT make up relations between node types in {node_labels} "
        f"that are not defined in {rel_types}.\n"
        if relationship_type == "tuple"
        else "",
    ...
    # print(f'Human Prompt String: {human_prompt_string}\n\n')
    ...
    # print(f'Human Message Prompt: {human_message_prompt}\n\n')
